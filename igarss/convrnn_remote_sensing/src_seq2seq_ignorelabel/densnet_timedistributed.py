...
def __conv_block(ip, nb_filter, bottleneck=False, dropout_rate=None, weight_decay=1E-4,
    ...
    if bottleneck:
        ...
        x = TimeDistributed(Conv2D(inter_channel, (1, 1), kernel_initializer='he_uniform', padding='same', use_bias=False,
...
def __create_fcn_dense_net(nb_classes, img_input, include_top, nb_dense_block=5, growth_rate=12,
    ...
    if concat_axis == 1:  # th dim ordering
        _, time, rows, cols = input_shape
    else:
        time, rows, cols, _ = input_shape
    ...
    # Initial convolution
    x = TimeDistributed(Conv2D(init_conv_filters, (3, 3), kernel_initializer="he_uniform", padding="same", name="initial_conv2D", use_bias=False,
